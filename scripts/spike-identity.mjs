...
for (const [label, envk] of HARNESS_SESSION_ENVS) {
  if (process.env[envk]) {
    harnessSession = { label, envk, id: process.env[envk] };
    break;
  }
}
...
let key, source;
if (explicitId) {
  key = `explicit:${explicitId}@${host}`;
  source = "explicit";
} else if (harnessSession) {
  key = `harness:${harnessSession.label}:${harnessSession.id}@${host}`;
  source = `harness (${harnessSession.envk})`;
} else if (ttyDevice) {
  key = `tty:${ttyDevice}@${host}`;
  source = isReal(selfTty) ? "tty (self)" : "tty (ancestry)";
} else {
  key = null;
  source = "none";
}
...
line(
  `  controlling tty  : self=${selfTty || "(none)"}  ancestry=${ancestryTtyRow ? ancestryTtyRow.tty : "(none)"}  → ${ttyDevice || "(none)"}`,
);
