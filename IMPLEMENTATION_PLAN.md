...
6. **Clean seams.** Persistence behind `Store`; identity behind `resolveSessionKey()`.
...
```
weaver/
├── package.json                # name "@<scope>/weaver", bin: { weaver: dist/cli.js }
├── tsconfig.json
...
│   ├── identity/
│   │   └── session.ts          # resolveSessionKey() + harness detection
│   ├── repo/
...
│   ├── glob/
│   │   └── match.ts            # path-vs-glob + glob-overlap heuristics (picomatch)
│   ├── conflict/
│   │   └── detect.ts           # 3-tier conflict detection over the store
│   ├── instructions/
│   │   └── block.ts            # the fenced CLAUDE.md/AGENTS.md block + inject/remove
│   ├── render/
│   │   ├── human.ts            # terse human output (silent-when-empty)
│   │   └── json.ts             # --json output shapes
│   └── dashboard/
│       ├── server.ts           # tiny HTTP + SSE server over the Store (read-only)
│       └── public/index.html   # single-page live view
├── scripts/
...
- [ ] **License.** Add an `MIT` `LICENSE` file (copyright Sean / Weaver contributors).
- [ ] **Versioning = SemVer**, starting at **`0.1.0`**. Treat `0.x` as pre-stable (minor
      bumps may break); cut `1.0.0` when the CLI surface is stable.
- [ ] **Conventional Commits.** Adopt `feat:` / `fix:` / `chore:` / `docs:` / `refactor:` /
      `test:` (already a project convention). This drives automated changelog + version bumps.
- [ ] **Automated releases via `release-please`.** A GitHub Action reads conventional commits
...
- [ ] **`CHANGELOG.md`** in Keep-a-Changelog format, maintained by `release-please`.
- [ ] **CI** (`.github/workflows/ci.yml`): lint + typecheck + test on every PR, under **both
      Node and Bun**. Branching stays simple: `main` is always releasable; work on branches → PRs.
- [ ] **npm publish config.** `package.json` `"version": "0.1.0"`, `"publishConfig": {
...
- [ ] **`repo/paths.ts` — target normalization.** Normalize all user-supplied paths/globs to
...
- [ ] **`store/store.ts` — the `Store` interface.** Methods the verbs need, e.g.
...
- [ ] **`store/sqlite.ts` — implement `Store` over `openDb`.** Prepared statements; epoch-ms
      timestamps passed in (never `Date.now()` inside pure logic — inject a clock for
      testability).
- [ ] **`store/reap.ts` — lazy staleness + retention.** `isStale(session, now, ttl)` and
...
- [ ] **Presence registration matrix.** Do not let human observer commands create fake live
      ...
      session hint.
- [ ] **`task`.** `weaver task "<intent>"` → set `sessions.intent`. Add a `task` activity row.
- [ ] **`claim` / `release`.** Normalize the target glob, then
...
- [ ] **`glob/match.ts`.** `matchesPath(glob, path)` via picomatch (precise over normalized
...
- [ ] **`conflict/detect.ts`.** Given a path/glob + the live store, return the highest tier
...
- [ ] **`check`.** Normalize the path/glob, run detection, print the descriptive payload;
...
- [ ] **`note` / `notes`.** Write a note (optional normalized `--path`, `--tag`, `--pin`);
      list notes (pinned first, newest first). Support `supersedes` when `--update <id>`.
- [ ] **`log`.** `weaver log <kind> <path> "<summary>"` → activity row with normalized target
      (kind validated).
- [ ] **`activity`.** Print recent activity (default last N), `--json` supported.
- [ ] **`status`.** Compose active sessions + active claims + recent activity + top/pinned
      ...
      shape via `render/json.ts`. **Cap output by default** (top-N recent activity, truncated
...
- [ ] **`done`.** End the caller's session (`ended_at`) and release its active claims.
- [ ] **`doctor`.** Print the resolved `{ key, source, label }`, repo-id + store path,
...
- [ ] **`instructions/block.ts`.** Export the exact fenced block (from the README — the
      ...
      target, not final.
- [ ] **`init`.** Resolve repo id, create/migrate the store, set `weaver_meta.enabled=1`,
...
- [ ] **`disable` / `enable`.** Toggle `weaver_meta.enabled`. When disabled,
      ...
      sessions.
- [ ] **`deinit`.** Remove the block from `CLAUDE.md`/`AGENTS.md`; with `--purge`, delete the
...
- [ ] **`dashboard/server.ts`.** Start a tiny HTTP server on `127.0.0.1` (configurable port,
...
- [ ] **`dashboard/public/index.html`.** Single page, vanilla JS, `EventSource('/events')`.
...
- [ ] **`dashboard` command.** Wire `weaver dashboard` (aliases `view`/`ui`) to launch the
      server in the foreground until `Ctrl-C`.
- [ ] **`watch` command.** Terminal TUI: clear+redraw the `status` view on a ~1s interval
...
- [ ] **Output polish.** Consistent terse formatting; friendly, descriptive conflict
      messages; `--help` for the binary and each verb; sensible exit codes.
- [ ] **Config & TTLs.** `weaver_meta.ttl_seconds` (session staleness) and a claim default
      TTL; allow `--ttl` overrides; document defaults (session ~5m, claim ~30m).
- [ ] **Errors & safety.** Support non-git directories via the cwd-hash repo-id fallback;
...
- [ ] **README finalization.** Ensure README matches shipped behavior; lead with the
      *con-texere* origin story.
- [ ] **Contributing.** Add `CONTRIBUTING.md` (setup, the Node+Bun test matrix, the PR bar).
      (`LICENSE`, versioning, and release plumbing were established in *Project foundations*.)
- [ ] **Demo / launch asset.** `scripts/demo.ts` spawns N simulated agents writing realistic
...
- [ ] **Distribution.** Publish `@<scope>/weaver` to npm with a `bin`. Add a
      ...
      runtime matrix.
- [ ] **CI/release green.** Confirm the `ci.yml` (from foundations) passes on Node & Bun, and
      that `release-please` cuts a clean `0.1.0` release PR + npm publish once the account exists.

*Phase acceptance:* a fresh machine can `npm i -g @<scope>/weaver`, `weaver init` a repo,
...
- **Unit (highest value):** `store` round-trips; `repo/paths` normalization;
  `glob/match` (path-vs-glob + overlap); `conflict/detect` (all four tiers, self/stale
...
1. **npm account/scope** — no npm username yet; create one before publishing (package will be
   `@<scope>/weaver`; the same handle can fill the `<gh-user>` install URL). Not a blocker for
   building.
2. ✅ **Arg parser** — hand-rolled, zero-dep (decided).
3. ✅ **Default TTLs** — session staleness ~5 min, claim ~30 min; tunable later via
   `weaver_meta` (decided).
4. **Target harnesses for v1** — drives which instruction files `init` writes and which the
   Phase-0 spike must cover. Working recommendation: `init` writes **CLAUDE.md + AGENTS.md**
   ...
   a clean TTY.
5. **Phase-0 outcome** — confirm TTY identity holds for the chosen harnesses, and decide
   whether any needs the explicit-id fallback mentioned in its instruction block.
